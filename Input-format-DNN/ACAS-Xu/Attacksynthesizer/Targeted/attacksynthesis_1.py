...
def find_delta(inputs_to_change, max_deltas):
    ...
    inputs = nn.addVars(inputSize, name="inputs")
    ...
    nn.addConstr(denormalizedOuts[1] <= denormalizedOuts[4])
    nn.addConstr(denormalizedOuts[4] <= denormalizedOuts[3])
    nn.addConstr(denormalizedOuts[3] <= denormalizedOuts[0])
